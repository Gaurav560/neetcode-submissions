class SuperHero:
    ...
    @health.setter
    def health(self,updated_health:int)->None:
        if 0<=updated_health<=100:
            self.__health=updated_health
        elif updated_health<0:
            print("You can't set the health to less than 0")   
        else:
            print("You can't set the health to more than 100") 

    @power_level.setter
    def power_level(self,updated_power_level)->None:
        if 1<=updated_power_level<=10:
            self.__power_level=updated_power_level
        elif updated_power_level>10:
            print("You can't set the power level to more than 10")
        elif updated_power_level<1:
